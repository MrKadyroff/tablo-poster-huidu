...
### 6.1.1 Флаги: одинаковый размер, скругление, верхний слой

Поля в `gridLayout` (все необязательные):

| Поле | По умолчанию | Что делает |
|---|---|---|
| `flagFit` | `"crop"` | Как флаг заполняет свою ячейку. `crop` — обрезка по центру, все флаги ровно `colFlagW × colFlagH` (одинаковая ширина). `pad` — старое поведение с полями. `stretch` — растянуть. |
| `flagRadius` | `2` | Скругление углов флага в пикселях табло. `0` — прямые углы. |
| `flagOnTop` | `false` | `true` — флаги рисуются последними, поверх всего. Ничто (текст, полосы, лого) их не перекрывает. |

В интерфейсе те же настройки — на вкладке «Дизайн», блок «Флаг и лого».

### 6.1.2 Анимация: сияние флагов и бегущая строка

Если включено сияние или бегущая строка, композитор пишет **`final.gif`** вместо `final.jpg`
(старый `final.jpg` удаляется, чтобы на табло уехала именно анимация) и отправляет его
как `picUnit fileType="gif"`. Без этих настроек всё работает как раньше — один JPEG.
...
"gridLayout": {
	...
	"animFrames": 96,
	"animDelayMs": 60,
	"shine": { "enabled": true, "count": 3, "strength": 0.8, "width": 0.35 },
	"ticker": {
		"enabled": true,
		"speed": 4,
		"bgColor": "#F58220",
		"textColor": "#ffffff",
		"langs": ["en", "kz", "ru", "tr", "zh", "ar"]
	}
}
```

- `shine` — блик, пробегающий по случайно выбранным флагам (`count` штук за рендер,
  на каждом рендере выбираются новые). `strength` — яркость 0..1, `width` — ширина блика
  в долях ширины флага.
- `ticker` — оранжевая полоса сверху (цвет лого eCash). Под неё отрезается `h` пикселей
  (по умолчанию ~14 % высоты), таблица сжимается в остаток — ничего не перекрывается.
  `langs` задаёт языки и их порядок: `en`, `kz`, `ru`, `tr`, `zh`, `ar`.
  `rates: true` добавит к каждой надписи список курсов, `text` — задаёт свой текст целиком.
- `animFrames` / `animDelayMs` — бюджет одного цикла GIF. Один цикл прокручивает строку
  ровно на её длину, поэтому скорость = длина текста / число кадров. Если текста больше,
  чем влезает в `animFrames` при `ticker.speed`, в лог уйдёт предупреждение: укоротите
  `langs`/`fontSize` или увеличьте `animFrames`.

**Скорость чтения.** Реальная скорость прокрутки = `ticker.speed / animDelayMs` пикселей в мс.
Чтобы замедлить строку, уменьшайте `speed` **и** поднимайте `animFrames` (иначе кадров не хватит
на цикл и скорость вернётся), либо увеличивайте `animDelayMs` — это замедляет без роста файла,
но при большом `speed` движение становится шагами. Обычно лучше менять оба параметра понемногу.

Размер файла: 560×80 при 96 кадрах ≈ 300 КБ. Больше кадров — плавнее и медленнее прокрутка,
но тяжелее файл; контроллер придётся проверить на своей скорости FTP.

Для шрифтов китайского и арабского используются системные шрифты Windows
(Microsoft YaHei / SimSun / Segoe UI / Arial Unicode MS). Если их нет, эти сегменты
не отрисуются — остальные языки продолжат работать.
